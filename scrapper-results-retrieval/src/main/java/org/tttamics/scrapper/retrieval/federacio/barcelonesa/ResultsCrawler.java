...
public class ResultsCrawler {
    ...
    private ResultsScratcher resultsScratcher;

    private ResultsWriter resultsWriter;

    @Inject
    public ResultsCrawler(ResultsScratcher resultsScratcher, @Qualifier("BcnResultsWriter") ResultsWriter resultsWriter) {
        this.resultsScratcher = resultsScratcher;
    ...
    public List<SeasonUrl> getSeasons() throws IOException {
        ...
        seasonUrlList = filterSeasons(seasonUrlList);
        return seasonUrlList;
    }

    private List<SeasonUrl> filterSeasons(List<SeasonUrl> seasonUrlList) {
        List<SeasonUrl> filteredSeasonUrlList1 = new ArrayList<>();

        for (SeasonUrl seasonUrl : seasonUrlList) {
            if ("2019-2020".equals(seasonUrl.getSeason())) {
                filteredSeasonUrlList1.add(seasonUrl);
            }
        }

        return filteredSeasonUrlList1;
    }
    ...
    public List<MatchRecord> fetch() throws IOException {
            ...
            for (CategoryUrl categoryUrl : categoryUrlList) {
                ...
                for (GroupUrl groupUrl : categoryUrl.getUrls()) {
                    ...
                    for (MatchUrl matchUrl: pdfUrlsList) {
                        ...
                        Map<String, String> scratchedData = resultsScratcher.scratch(matchUrl);
